...
export const CURRICULUM: PathNode[] = [
  ...
  { id: 't-hanging-set', track: 'tactics', band: 'foundation', kind: 'themed-set', xp: 60, requires: ['t-hanging'], theme: 'hangingPiece', target: { count: 10, accuracy: 70 }, title: 'Drill: hanging pieces', blurb: 'Solve 10 hanging-piece puzzles at 70 %+ accuracy.' },
  {
    ...
    steps: [
      ...
      { text: 'The **royal fork** — check plus an attack on the queen or rook — is the strongest version because the king *must* respond.', task: { fen: 'r3k3/8/8/3N4/8/8/8/4K3 w - - 0 1', prompt: 'White to move. Fork the king and rook — and take it.', solution: ['d5c7', 'e8d7', 'c7a8'], explain: 'Nc7+ forks king and rook; the king must move, and Nxa8 collects.' } },
      { text: 'Queens fork too — but the second target must be hit on a **different line** than it defends. Attack a rook along its own file or rank and it will simply capture the queen.', task: { fen: 'r3k3/8/8/8/7Q/8/8/4K3 w - - 0 1', prompt: 'White to move. Check and win the rook.', solution: ['h4e4', 'e8d7', 'e4a8'], explain: 'Qe4+! checks down the e-file and hits a8 on the long diagonal — the rook cannot capture across a diagonal — and after the king moves, Qxa8. (Qa4+? and Qh8+? look like forks, but the rook just plays Rxa4 / Rxh8 along its own line.)' } },
    ],
  },
